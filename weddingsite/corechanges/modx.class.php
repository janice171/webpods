...
class modX extends xPDO {
    ...
    public function makeUrl($id, $context= '', $args= '', $scheme= -1) {
        ...
        if ($validid = intval($id)) {
			...
			$e = new Exception();
            $backtrace = print_r(str_replace('/path/to/code/', '', $e->getTraceAsString()), true);
			/*ob_start();
			debug_print_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
			$backtrace = ob_get_contents();
			ob_end_clean();*/
            $this->log(modX::LOG_LEVEL_ERROR, '`' . $id . "` is not a valid integer and may not be passed to makeUrl() ===> $backtrace");
    ...
    public function invokeEvent($eventName, array $params= array ()) {
       ...
       if ($this->eventMap === null)
        ...
        if (!isset ($this->eventMap[$eventName])) {
            //$this->log(modX::LOG_LEVEL_ERROR,'System event '.$eventName.' was executed but does not exist.');
        ...
        if (count($this->eventMap[$eventName])) {
            ...
            foreach ($this->eventMap[$eventName] as $pluginId => $pluginPropset) {
                ...
                if ($plugin && !$plugin->get('disabled')) {
                    ...
                    //SJH dont do this if ($msg && is_string($msg)) {
