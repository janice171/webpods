 ...
 * 
 * @class WSSM.grid.User
 * @extends MODx.grid.Grid
 * @param {Object} config An object of config properties
 * @xtype wssm-grid-user
 */
WSSM.grid.User = function(config) {
    config = config || {};
	Ext.applyIf(config,{
		...
		,tbar: [{
            ...
            ,emptyText: _('wssm_filter_by_lastname')
            ,listeners: {
                'change': {fn:this.filterByName,scope:this}
                ,'render': {fn:function(tf) {
                    tf.getEl().addKeyListener(Ext.EventObject.ENTER,function() {
                        this.filterByName(tf); 
                    },this);
                },scope:this}
            }
        }]
	});
	WSSM.grid.User.superclass.constructor.call(this,config);
};

Ext.extend(WSSM.grid.User,MODx.grid.Grid,{
    ...
    ,updateUser: function() {
        location.href = '?a='+MODx.request.a+'&action=updateUser'+'&id='+this.menu.record.id;
...
Ext.reg('wssm-grid-user',WSSM.grid.User);
